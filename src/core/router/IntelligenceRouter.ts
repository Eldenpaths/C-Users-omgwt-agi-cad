...
import { setAgentOverride, getSnapshot, recordOutcome } from '@/lib/routerWeights'
  ...
  async routeTask(task: RouteTask | Record<string, any>): Promise<any> {
    ...
    try {
      ...
      try {
        ...
        if (uid) {
          try {
            const prof = await getFullProfile(String(uid))
            const cfg = await getPolicy(String(uid))
            const deltaAvg = computeRollingDelta(prof?.rewards || [], cfg.window)
            const snap2 = getSnapshot()
            const adjusted = applyPolicyAdjustments({ ...snap2.agents }, deltaAvg, cfg)
            for (const key of Object.keys(adjusted)) {
              const bias = (adjusted as any)[key]?.bias
              if (typeof bias === 'number') setAgentOverride(key as any, { bias })
            }
            await setPolicy(String(uid), { lastAutoTune: Date.now() })
          } catch {}
        }
      } catch {}
      return result
